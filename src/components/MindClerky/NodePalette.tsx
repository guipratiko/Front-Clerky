...
interface NodePaletteProps {
  onAddNode: (type: 'whatsappTrigger' | 'typebotTrigger' | 'condition' | 'delay' | 'end' | 'response' | 'spreadsheet' | 'openai') => void;
}
...
export const NodePalette: React.FC<NodePaletteProps> = ({ onAddNode }) => {
  ...
  const nodeTypes = [
    ...
    },
    {
      ...
      label: t('mindClerky.nodes.openai'),
      description: t('mindClerky.nodes.openai.description'),
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
