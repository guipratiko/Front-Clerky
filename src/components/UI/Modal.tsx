...
const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div
      className={`fixed inset-0 ${draggable ? 'pointer-events-none' : 'flex items-center justify-center p-4'} ${draggable ? '' : 'bg-black/60 backdrop-blur-sm'} animate-fadeIn`}
      style={{ zIndex }}
      onClick={draggable ? undefined : onClose}
    >
      <div
        ref={modalRef}
        className={`bg-white dark:bg-gray-800 rounded-xl shadow-2xl ${draggable ? '' : 'w-full'} ${sizeClasses[size]} max-h-[90vh] overflow-hidden flex flex-col ${draggable ? 'pointer-events-auto' : 'animate-slideIn'}`}
        style={modalStyle}
